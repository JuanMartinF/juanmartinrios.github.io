...
# Rutas de entrada
catalogo_path = "catalogo_de_cuadrosG.csv"
template_path = "html/g01.html"
...
for _, row in df.iterrows():
    ...
    if existing_html == 'no':
        ...
        html = html.replace("g01_web.jpg", f"{codigo}_web.jpg")
        html = html.replace("Up", nombre)
        html = html.replace("Up", nombre)
        html = html.replace("14.941€", str(precio))
        html = html.replace("184x145 cm", str(dimensiones))
        html = html.replace("g01", codigo)
...
print("Archivos generados en la carpeta 'proyectos/proyectoG/html/prueba'")
